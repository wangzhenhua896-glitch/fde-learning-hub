...
(function () {
  ...
  function dimState(p, dim) {
    ...
    if (st) {
      if (st.status === 'passed') return { s: 'passed', from: '阶段「' + stageTitle(p, sid) + '」验收通过，证据 ' + latestVersion(p, sid).evidN + ' 条' };
      if (st.status === 'submitted' || st.status === 'revise') return { s: 'review', from: '阶段「' + stageTitle(p, sid) + '」' + ST_LABEL[st.status] + '，评审通过后升级' };
  ...
  var view = $('#wb-view');
  var TABS = [
    ['home', '📚 实训库'], ['today', '📅 今日'], ['diag', '📋 诊断'], ['skills', '🗺 能力地图'], ['data', '💾 数据']
  ];
  ...
  function renderHome() {
    var p = profile();
    var total = D.projects.reduce(function (a, pr) { return a + pr.stages.length; }, 0);
    var passed = totalPassed();
    var html = head('实训库', '5 个实训项目 · ' + total + ' 个阶段任务包。每个项目走完整的「简报→任务→诊断→辅导→提交→评审」闭环，验收通过计入能力档案。');
    if (!p) {
    ...
    html += '<div class="wb-progress"><div class="wb-progress-bar"><i style="width:' + (passed / total * 100) + '%"></i></div>' +
      '<span>全部项目累计 ' + passed + ' / ' + total + ' 阶段通过 · 目标：' + esc(p.goal) + '</span></div>';
    ...
    D.projects.forEach(function (pr, i) {
      ...
      html += '<div class="wb-card' + (i === 0 && done === 0 ? ' focus' : '') + '"><div class="wb-card-tag">' + esc(pr.tier) + ' · ' + esc(pr.tag) + '</div>' +
        ...
        (allDone(pr) ? '<a class="wb-btn pri" href="#p/' + pr.id + '">复习与导出 →</a>' :
          '<a class="wb-btn pri" href="#p/' + pr.id + '">' + (done > 0 ? '继续：' + esc(ns.title) + ' →' : '开始项目 →') + '</a>');
      html += '</div>';
    });
    html += '</div>';
    html += '<p class="wb-note">进度与证据只保存在你自己的浏览器（localStorage）。跨项目的能力聚合与导出分享见 <a href="profile.html">能力档案 →</a>；数据备份见各项目的「数据」页。</p>';
    view.innerHTML = html;
  }
  ...
  function renderToday(p) {
    ...
    html += '<div class="wb-progress"><div class="wb-progress-bar"><i style="width:' + (doneCount(p) / p.stages.length * 100) + '%"></i></div>' +
      '<span>' + doneCount(p) + ' / ' + p.stages.length + ' 阶段通过</span></div>';
    if (ns) {
      ...
      html += '<div class="wb-card focus"><h3>🎉 本项目全部 ' + p.stages.length + ' 个阶段验收通过</h3><p>你已获得「' + esc(p.title) + '」完整作品集。可到「数据」页导出证据，或到 <a href="profile.html">能力档案</a> 查看跨项目聚合。</p></div>';
    }
    html += '<div class="wb-grid2">' +
      ...
      '<div class="wb-card"><h3>🗺 能力地图</h3><p>六个维度的证据与差距：每个维度显示当前状态、证据来源、以及"补哪一个任务"。</p><a class="wb-btn" href="#p/' + p.id + '/skills">查看能力地图</a></div></div>';
    html += '<p class="wb-note">状态保存在本浏览器（localStorage），可到「数据」页导出备份。评审方式：确定性格式检查 + 量表逐项证据留痕；模型评审与隔离执行将在后续版本提供。</p>';
    view.innerHTML = html;
  }
  ...
  function renderDiag(p) {
    ...
    html += head('基础诊断 · ' + esc(p.title), p.quiz.length + ' 道题 × 6 个维度。诊断结果只作为学习线索：<b>答错不会给你打低分，只标记该维度"未评估/需补课"</b>；能力状态以阶段作品的证据为准。');
  ...
  function renderReview(p, sid, s, v) {
    var html = '<div class="wb-review"><h4>评审：逐项给出证据</h4>' +
      '<p class="wb-note">评审方式说明：采用<b>量表评审 + 证据留痕</b>（每项须写出你如何满足、证据在哪），全部项有证据 + 变式追问回答后转为「通过」。</p>';
  ...
  function renderSkills(p) {
    var html = '<div class="crumb"><a href="#p/' + p.id + '">' + esc(p.title) + '</a><span>/</span><span>能力地图</span></div>';
    html += head('能力地图 · ' + esc(p.title), '六个维度 × 四种状态（未评估 / 练习中 / 待复核 / 有证据通过）。避免伪精确的百分比：只有阶段验收证据能提升状态。跨项目聚合见 <a href="profile.html">能力档案</a>。');
    ...
    D.dims.forEach(function (d) {
      ...
      html += '<div class="wb-scard"><div class="wb-scard-h"><b>' + esc(d.name) + '</b><span class="' + DIM_CLS[st.s] + '">' + DIM_ST[st.s] + '</span></div>' +
        ...
        '<div class="wb-sact">' + (stageDone ? '<span class="wb-done-mark">✅ 阶段「' + esc(stageTitle(p, sid)) + '」已通过</span>' :
          '<a href="#p/' + p.id + '/task-' + sid + '" class="wb-btn sm">补：阶段「' + esc(stageTitle(p, sid)) + '」→</a>') + '</div></div>';
    });
    html += '</div>';
    html += '<section class="wb-sec"><h3>证据台账（本项目）</h3><p class="wb-note">每条证据包含：任务、版本、评审依据、是否在帮助下完成、日期。跨项目台账见 <a href="profile.html">能力档案</a>；可到「数据」页导出。</p><ul class="wb-evlist">';
    var any = false;
    ...
    if (!any) html += '<li class="wb-note">本项目暂无通过记录。完成第一个阶段的提交—反馈—修改—通过循环后，这里会出现第一条证据。</li>';
    html += '</ul></section>';
    view.innerHTML = html;
  }
